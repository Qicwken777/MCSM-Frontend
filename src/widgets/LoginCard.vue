...
const handleSsoLogin = () => {
  // 这是整页跳转（OAuth 授权码流程），不走 axios，不会被 apiService.ts 的
  // getApiBase() 处理，必须在这里单独拼接后端地址，否则跨域部署时会
  // 跳到前端自己的域名，而不是真正处理 SSO 的后端。
  const apiBase = getApiBase();
  window.location.href = `${apiBase}/api/auth/sso/authorize`;
};
